 ...
 */
export function resolveTavilySettings(value) {
 ...
 */
export function validateTavilyApiKey(key) {
 ...
 */
export function buildTavilyRequestBody(query, settings) {
    return {
...
function projectTavilySource(item, includeRaw) {
 ...
 */
export function mapTavilyResponse(json, includeRaw) {
    ...
    for (const entry of rawResults) {
        if (typeof entry !== "object" || entry === null)
            continue;
        const source = projectTavilySource(entry, includeRaw);
        if (source === null || seen.has(source.url))
            continue;
        seen.add(source.url);
        sources.push(source);
    }
 ...
 */
export function formatTavilyOutput(result) {
    const parts = [];
    if (result.content !== void 0 && result.content.length > 0)
