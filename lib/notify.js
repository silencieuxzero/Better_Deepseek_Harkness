 ...
 */
export function resolveNotifySettings(value) {
...
export function platformIsWindows(platform) {
 ...
 */
export function escapePowerShellSingleQuoted(text) {
...
export function notifyTitleFor(kind) {
 ...
 */
export function buildQuestionBody(questions, cap = NOTIFY_BODY_CAP) {
 ...
 * @returns the summary body.
 */
export function buildDoneBody(outcome, cap = NOTIFY_BODY_CAP) {
...
/**
 * Build the complete Windows PowerShell toast script. Title and body are
 * embedded as single-quoted literals (quotes doubled) and appended through
 * `CreateTextNode`, so no further XML escaping is needed and the script is
 * safe to ship through `-EncodedCommand` (UTF-16LE base64).
 *
...
export function buildToastScript(title, body) {
    const literal = (text) => `'${escapePowerShellSingleQuoted(text)}'`;
    return [
        ...
        onStatus(agentId, status, now = Date.now()) {
            if (status === "running") {
                if (agents.has(agentId))
                    return null; // already running — keep the original start
