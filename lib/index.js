 ...
 */
function thirdPartyBundleLayers(layout, ctx, ownName) {
  ...
  for (const name of bundles) {
    ...
    if (rowIds.length > 0) layers.push({ name, rowIds });
  }
  return layers;
}
...
function applyRescue(cx, failure) {
  ...
  const plan = buildRescuePlan(list, thirdPartyBundleLayers(layout, ctx, NAME), NAME, liveEntriesOf(ctx), failure);
  if (!plan.changed) {
...
function scheduleRescueSettle(cx, boot) {
  ...
  const timer = setTimeout(() => {
    ...
    const problems = startupProblems(loadPatchListSafe(layout), liveEntriesOf(ctx), NAME, {
      includePending: true,
      extraIds: thirdPartyBundleLayers(layout, ctx, NAME).flatMap((layer) => layer.rowIds)
    });
