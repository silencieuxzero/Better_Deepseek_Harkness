...
/** Skill Markdown download ceiling (security invariant — URL installs never grow unbounded). */
const SKILL_DOWNLOAD_LIMIT = 1024 * 1024; // 1 MiB
/** npm tarball download ceiling (security invariant — URL/npm installs never grow unbounded). */
const PACKAGE_TARBALL_LIMIT = 64 * 1024 * 1024; // 64 MiB
...
function readConfig(ctx) {
  ...
  const notify = storedSection.notify && typeof storedSection.notify === "object" && !Array.isArray(storedSection.notify)
    ...
    : { ...DEFAULTS.notify };
  // Top-level scalars are fail-closed / fall-back for hand-edited
  // settings.yaml: an invalid `allowLan` must never widen API access, and an
  // invalid path list must not crash the state snapshot or skill scanner.
  return {
    ...DEFAULTS,
    ...storedSection,
    allowLan: storedSection.allowLan === true,
    skillRoot: typeof storedSection.skillRoot === "string" ? storedSection.skillRoot : DEFAULTS.skillRoot,
    treeRoot: typeof storedSection.treeRoot === "string" ? storedSection.treeRoot : DEFAULTS.treeRoot,
    customSkillDirs: Array.isArray(storedSection.customSkillDirs)
      ? storedSection.customSkillDirs.filter((dir) => typeof dir === "string")
      : [...DEFAULTS.customSkillDirs],
    vision,
    tavily,
    github,
    notify
 ...
 * Returning `false` is the reserved "planned but nothing to write" signal:
 * the caller still ran inside the serialized slot, but the patch file is
 * left untouched (used by rescue no-change plans).
 */
...
/** True when a loader entry is actually running (ACTIVE, not disabled). */
function loaderEntryActive(entry) {
  return entry !== void 0 && entry !== null && entry.disabled !== true && entry.fiber?.state === 2;
}

/** True when a removed entry is no longer live (absent, disposed, or unloading). */
function loaderEntryGone(entry) {
  return entry === void 0 || entry === null || entry.fiber?.state === 4 || entry.fiber?.state === 5;
}
...
/**
 * Read one fetch response into a Buffer, cancelling the stream as soon as
 * `limit` is exceeded. Real fetch bodies are web ReadableStreams; mock
 * responses with no stream fall back to arrayBuffer so tests stay simple.
 */
async function readResponseBuffer(res, limit, code, message) {
  ...
  if (typeof res.arrayBuffer === "function") {
    ...
    if (buf.length > limit) throw err(code, message);
    return buf;
  }
  throw err("download-failed", "response body is not readable");
}
...
async function fetchText(url) {
  ...
  const buf = await readResponseBuffer(res, SKILL_DOWNLOAD_LIMIT, "download-too-large", `skill download exceeds ${SKILL_DOWNLOAD_LIMIT} bytes`);
...
async function downloadTarball(url) {
  ...
  if (!res.ok) throw err("download-failed", `download of ${url} returned HTTP ${res.status}`);
  const buf = await readResponseBuffer(res, PACKAGE_TARBALL_LIMIT, "download-too-large", `plugin tarball exceeds ${PACKAGE_TARBALL_LIMIT} bytes`);
 ...
 * from `main` and `exports` (shorthand string, or `exports["."]` as a
 * string, fallback array, or condition object with import/require/default).
 * Returns [] when nothing is declared — then the loader's own resolution
 * decides and no build is attempted.
 */
function packageEntryPoints(manifest) {
  const out = [];
  if (typeof manifest.main === "string" && manifest.main.trim() !== "") out.push(manifest.main);
  // `exports` is normally the condition-object form (`{".": ...}`), but Node
  // also accepts the shorthand string form (`"exports": "./index.js"`).
  if (typeof manifest.exports === "string" && manifest.exports.trim() !== "") {
    out.push(manifest.exports);
    return out;
  }
  ...
  switch (source.kind) {
    case "folder": {
      if (typeof source.path !== "string" || source.path.trim() === "") {
        throw err("source-not-found", "folder source requires a non-empty path");
      }
      const src = resolve(source.path.trim());
...
function bundlePatchRows(pkgDir) {
  ...
  if (abs !== packageRoot && !abs.startsWith(packageRoot + sep)) {
    throw err("bundle-patch-invalid", `dsh.bundle.patch must stay inside the package directory: ${rel}`);
  }
...
function readBody(req) {
  return new Promise((resolvePromise, reject) => {
    ...
    req.on("data", (chunk) => {
      total += chunk.length;
      if (total > BODY_LIMIT) {
        // Reject but do NOT destroy the socket here: the dispatcher sends a
        // structured 413 response. Pausing + dropping the data listener stops
        // the hostile upload from buffering further while the response goes out.
...
/**
 * Discard worktree edits and delete untracked files (only when still
 * untracked). A path can carry both staged and unstaged edits in one
 * porcelain record; the caller therefore says which row it discarded:
 *   - `staged: true`  → reset index AND worktree to HEAD (a "discard" on the
 *     staged row removes the whole change);
 *   - `staged: false` → restore only the worktree from the index (the staged
 *     version, when one exists, is preserved).
 */
async function gitDiscard(body, cx) {
  const sessionId = gitSessionId(body, void 0);
  const { root, state } = await gitStatusOf(cx, sessionId);
  const paths = checkGitPaths(body, root);
  const discardStaged = body.staged === true;
  const untracked = new Set(state.changes.filter((entry) => entry.untracked).map((entry) => entry.path));
  const checkout = [];
  const stagedReset = [];
  const untrackedDelete = [];
  for (const path of paths) {
    if (untracked.has(path)) {
      ...
      untrackedDelete.push(abs);
    } else if (discardStaged) {
      stagedReset.push(path);
    } else {
      checkout.push(path);
    }
  }
  if (stagedReset.length > 0 && !(await hasGitHead(root, cx.cfg.git.timeoutMs))) {
    throw err("git-discard-head", "cannot discard staged changes before the first commit — unstage the file first");
  }
  if (checkout.length > 0) await runGit(root, ["checkout", "--", ...checkout], cx.cfg.git.timeoutMs);
  if (stagedReset.length > 0) {
    await runGit(root, ["restore", "--staged", "--worktree", "--", ...stagedReset], cx.cfg.git.timeoutMs);
  }
  // Untracked files are deleted last so a failed git command leaves them
  // intact (deleting first would make a failed batch partially destructive).
  for (const abs of untrackedDelete) rmSync(abs, { force: true });
  return { discarded: paths, deletedUntracked: untrackedDelete.length, discardedStaged: stagedReset.length };
}
...
// A leading "-" is invalid in a git branch name, but `git checkout` would
// still parse it as an option (e.g. --detach) before validating the ref, so
// the whitelist must reject it explicitly. The trailing "--" is a second
// guard: even a future refname edge case can never reach git's option parser.
const GIT_BRANCH_RE = /^[A-Za-z0-9._@+/~#][A-Za-z0-9._@+/~#-]{0,199}$/;
...
async function mcpAdd(body, cx) {
  ...
  const applied = await waitForLoaderState(ctx, (entries) => {
    for (const entry of entries) if (String(entry.id) === rowId) return loaderEntryActive(entry);
    return false;
  });
...
async function mcpRemove(body, cx) {
  ...
  const applied = await waitForLoaderState(ctx, (entries) => {
    for (const entry of entries) if (String(entry.id) === rowId) return loaderEntryGone(entry);
    return true;
  });
...
async function mcpSetEnabled(body, cx) {
  ...
  const applied = await waitForLoaderState(ctx, (entries) => {
    for (const entry of entries) {
      if (String(entry.id) !== rowId) continue;
      return enabled ? loaderEntryActive(entry) : entry.disabled === true;
    }
    return false;
  });
...
function contentHasImageBlocks(content) {
  ...
  return content.some((block) => block && typeof block === "object"
    && (block.type === "image" || block.type === "tool-result" && contentHasImageBlocks(block.content)));
}
...
function needsTranscription(options, vision) {
  ...
  return options.messages.some((message) => message && typeof message === "object" && contentHasImageBlocks(message.content));
}
...
async function transcribeRequest(ctx, llm, options, vision) {
  ...
  for (const message of messages) {
    if (!message || typeof message !== "object" || !contentHasImageBlocks(message.content)) continue;
    const images = [];
    const walk = (content) => {
  ...
  return { ...options, messages: messages.map((message) => ({ ...message, content: rewriteContent(message.content, replacements) })) };
}
...
/**
 * Combine the harness's cooperative abort signal with this tool's own hard
 * timeout. The hard timeout must also apply when the runner supplies no
 * signal at all — a tool call may be wrapped by runners that omit `exec.signal`.
 */
function toolFetchSignal(exec, timeoutMs) {
  if (typeof AbortSignal.any === "function") {
    const timeout = AbortSignal.timeout(timeoutMs);
    return exec?.signal !== void 0 ? AbortSignal.any([exec.signal, timeout]) : timeout;
  }
  if (exec?.signal !== void 0) return exec.signal;
  // Older runtimes have timeout() but not any(): still enforce the hard
  // timeout when the runner supplied no cooperative signal.
  return typeof AbortSignal.timeout === "function" ? AbortSignal.timeout(timeoutMs) : void 0;
}
...
function tavilyToolDefinition(configOf) {
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      const signal = toolFetchSignal(exec, TAVILY_TOOL_TIMEOUT_MS);
      let response;
      try {
...
async function githubApiCall(configOf, url, exec) {
  ...
  const signal = toolFetchSignal(exec, settings.timeoutMs);
  let response;
  try {
...
/**
 * Apply rescue through the serialized patch writer. This is the manual /
 * settle-window path: the boot-time `applyRescue()` must stay synchronous to
 * win the race with the harness boot audit, but every later invocation can
 * share the same writer as plugin install / MCP / resolve mutations.
 */
async function applyRescueSerialized(cx, failure) {
  const { layout, ctx } = cx;
  const now = Date.now();
  const boot = { pid: rescuePid(), startedAt: now, healthy: false, healthyAt: null };
  const log = (message, ...args) => ctx.logger?.warn?.(message, ...args);
  ...
  let outcome;
  try {
    outcome = await withPatchWrite(layout, (list) => {
      const plan = buildRescuePlan(list, layers, NAME, liveEntriesOf(ctx), failure, protect);
      if (!plan.changed) return false; // planned inside the lock — nothing to write
      list.splice(0, list.length, ...plan.updatedList);
      return { plan };
    });
  } catch (error) {
    log("better-deepseek-harness: serialized rescue patch write failed: %s", error?.message ?? error);
    return { applied: false, reason: "patch-write-failed" };
  }
  if (outcome === false) {
    log("better-deepseek-harness: rescue mode has no third-party plugins to disable — staying in normal mode");
...
function scheduleRescueSettle(cx, boot) {
  ...
  const timer = setTimeout(() => {
    ...
    const run = async () => {
      ...
      const { layers, protect } = thirdPartyBundleLayers(layout, ctx, NAME, cx.cfg.rescue.protectBundles);
      if (state.phase === "applied") await reviseAppliedRescue(cx, state, layers, protect);
      ...
      if (problems.length > 0) {
        await applyRescueSerialized(cx, problems[0]);
        return;
      }
      // Re-read after any awaited patch work: a concurrent resolve may have
      // replaced the boot record.
      const latest = loadRescueState(layout);
      if (latest.boot.pid !== boot.pid || latest.boot.startedAt !== boot.startedAt) return;
      saveRescueState(layout, markBootHealthy(latest, Date.now()));
    };
    run().catch((error) => {
      ctx.logger?.warn?.("better-deepseek-harness: rescue settle check failed: %s", error?.message ?? error);
    });
 ...
 */
async function reviseAppliedRescue(cx, state, layers, protect) {
  const { layout, ctx } = cx;
  try {
    let plan = null;
    await withPatchWrite(layout, (list) => {
    ...
    // Do not clobber a concurrent resolve that already cleared the state.
    const latest = loadRescueState(layout);
    if (latest.phase !== "applied") return;
    const merged = [...latest.plugins];
    for (const plugin of plan.plugins) {
      if (!merged.some((entry) => entry.name === plugin.name)) merged.push(plugin);
    }
    saveRescueState(layout, { ...latest, plugins: merged });
    ctx.logger?.warn?.(
...
async function applyRescueCommandText(cx, names) {
  ...
  // `none` is an explicit "keep everything disabled" choice: like an empty
  // selection it must close the rescue cycle WITHOUT scheduling a reload.
  const enable = names.length === 0 || names.includes("none") ? [] : names.includes("all") ? all : names;
    ...
    ctx.effect(() => commands.register({
      ...
      handler: async (invocation) => {
        ...
        try {
          ...
          if (verb === "trigger") {
            const outcome = await applyRescueSerialized(cx, {
              kind: "manual",
...
function apply(ctx, config = {}) {
  ...
  const handle = async (req, res) => {
    ...
    try {
      ...
      const status = error?.code === "payload-too-large" ? 413 : 400;
      return send(res, status, { ok: false, error: { code: error.code || "bad-request", message: error.message } }, status === 413 ? { connection: "close" } : {});
    }
    let outcome;
    try {
...
/**
 * Resolve and validate one archived session's durable log directory. The
 * locate result must be an ABSOLUTE path that names a file in a session
 * directory — never a relative path (which would delete a cwd-relative
 * directory) and never a filesystem root.
 */
function archivedSessionDir(persistence, header) {
  const loc = persistence.locate(header);
  if (!loc || typeof loc.path !== "string" || loc.path.length === 0 || !isAbsolute(loc.path)) {
    throw err("archive-locate", "cannot locate the stored log for session " + header.id);
  }
  const dir = dirname(resolve(loc.path));
  if (dir === resolve(dirname(dir))) {
    throw err("archive-locate", "refusing to delete the filesystem root for session " + header.id);
  }
  return dir;
}
...
async function deleteArchivedSessions(body, cx) {
  ...
  const headers = Array.isArray(listed) ? listed.filter((header) => header && typeof header === "object" && typeof header.id === "string" && header.id !== "") : [];
  ...
  // Preflight every locate BEFORE deleting anything: a malformed locate result
  // then fails the whole batch instead of leaving it half-deleted.
  const sessionDirs = [];
  for (const id of deletable) {
    sessionDirs.push({ id, dir: archivedSessionDir(persistence, byId.get(id)) });
  }
  for (const entry of sessionDirs) {
    // The JSONL backend owns one directory per session; deleting it removes
    // the transcript plus any session-owned artifacts.
  ...
  const installed = Object.entries(state)
    .map(([name, record]) => {
      ...
      return {
        ...
        enabled: !(safe.disabled === true),
        builtin: safe.builtin === true,
        rows: Array.isArray(safe.rows) ? safe.rows.length : 0
      };
    })
    ...
    llmProviders: (() => {
      ...
      if (llm && typeof llm.listProviders === "function") {
        ...
        if (Array.isArray(providers)) {
          return providers.map((p) => ({ id: String(p?.id ?? ""), name: String(p?.name ?? p?.id ?? "") }));
        }
      }
      return [];
    })(),
...
const routes = {
      ...
      for (const key of ["allowLan", "skillRoot", "customSkillDirs", "treeRoot", "vision", "tavily", "github", "notify"]) {
        if (key in body) {
          ...
          if (key === "tavily") {
            ...
            if ("enabled" in value && typeof value.enabled !== "boolean") throw err("bad-request", "vision.enabled must be a boolean");
            for (const field of ["provider", "model", "prompt", "apiUrl"]) {
              if (field in value && typeof value[field] !== "string") throw err("bad-request", `vision.${field} must be a string`);
            }
            // The API key is write-only from the client's perspective: only a
            // non-empty trimmed string updates the stored key; blank, absent,
  ...
  "/ext/api/plugin/install": {
    ...
    handler: async (body, cx) => {
      ...
      try {
        ...
        mkdirSync(dirname(target), { recursive: true });
        // Preserve the previous install until the patch transaction below has
        // succeeded. A failed reinstall must never leave the old package gone.
        if (existsSync(target)) {
          backupDir = `${target}.backup-${randomUUID()}`;
        ...
        try {
          ...
          // The replacement itself failed — put the previous package back.
          if (backupDir) {
            try {
              rmSync(target, { recursive: true, force: true, maxRetries: 5, retryDelay: 200 });
              ...
              ctx.logger?.warn?.("better-deepseek-harness: failed to restore previous package %s: %s", target, rollbackError?.message ?? rollbackError);
            }
          }
          throw error;
        }
      ...
      try {
        ...
        // A failed patch transaction must not leave a half-tracked package on
        // disk: restore the previous install when one was backed up, otherwise
        // remove the newly-copied untracked package (fresh install / manually
        // deleted previous directory).
        if (backupDir) {
          try {
            rmSync(target, { recursive: true, force: true, maxRetries: 5, retryDelay: 200 });
            renameSync(backupDir, target);
          ...
          try {
            ...
            ctx.logger?.warn?.("better-deepseek-harness: failed to remove untracked package %s: %s", target, rollbackError?.message ?? rollbackError);
          }
        }
        throw error;
      }
      // Patch, state, and profile manifest are durable now — the previous
      // install can be discarded.
      if (backupDir) {
        try {
          rmSync(backupDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 200 });
          backupDir = null;
        } catch (error) {
          ctx.logger?.warn?.("better-deepseek-harness: failed to remove install backup %s: %s", backupDir, error?.message ?? error);
        }
      }
      const rowIds = patchListIds(record.rows);
      const applied = await waitForLoaderState(ctx, (entries) => {
        if (rowIds.length === 0) return entries.some((entry) => entry.options?.name === pkgName && loaderEntryActive(entry));
        return rowIds.every((id) => {
          const entry = entries.find((candidate) => String(candidate.id) === id);
          return loaderEntryActive(entry);
        });
      });
      return {
        name: pkgName,
  ...
  "/ext/api/plugin/uninstall": {
    ...
    handler: async (body, cx) => {
      ...
      const applied = await waitForLoaderState(ctx, (entries) => {
        if (rowIds.length === 0) return !entries.some((entry) => entry.options?.name === name && !loaderEntryGone(entry));
        return !entries.some((entry) => rowIds.includes(String(entry.id)) && !loaderEntryGone(entry));
      });
    ...
    handler: async (body, cx) => {
      ...
      const applied = await waitForLoaderState(ctx, (entries) => {
        if (rowIds.length === 0) return entries.some((entry) => entry.options?.name === name
          && (enabled ? loaderEntryActive(entry) : entry.disabled === true));
        return rowIds.every((id) => {
          const entry = entries.find((candidate) => String(candidate.id) === id);
          return enabled ? loaderEntryActive(entry) : entry?.disabled === true;
        });
...
export { NAME, SETTINGS_NS, apply, inject, materializePackage, packageEntryPoints, packageEntryExists, ensureBuiltPackage, __setRescueHostHooks, __setNotifyHostHooks, loaderEntryActive as __loaderEntryActive, loaderEntryGone as __loaderEntryGone };
