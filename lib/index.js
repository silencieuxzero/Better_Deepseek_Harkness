...
async function collectStreamText(stream) {
  ...
  for await (const chunk of stream) {
    ...
    if (chunk.type === "text-delta" && typeof chunk.text === "string") {
      ...
      // Defensive fallback for adapters/middleware that emit whole text blocks.
      text += chunk.text;
...
function registerTavilySearch(ctx, configOf) {
  const state = { tool: null, section: null };
  const sync = (config) => {
    let enabled = false;
    try {
    ...
    if (enabled) {
      ...
      if (state.section === null && typeof ctx.inject === "function") {
        try {
          ctx.inject(["systemPrompt"], (sctx) => {
            // The fiber may resolve after the setting flipped off, or a second
            // sync may already own the section — re-check before registering.
            let stillEnabled = false;
            try {
              stillEnabled = resolveTavilySettings(configOf().tavily).enabled;
            } catch { /* unreadable settings */ }
            if (!stillEnabled || state.section !== null) return;
