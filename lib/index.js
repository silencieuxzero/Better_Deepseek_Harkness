...
function patchEntryIds(list) {
  ...
  for (const row of list) {
    ...
    for (const entry of row.insert) if (entry && typeof entry === "object" && entry.id) ids.add(String(entry.id));
  }
  return ids;
}

/** All entry ids carried by a sidecar row snapshot. */
function rowEntryIds(rows) {
  const ids = [];
  for (const row of Array.isArray(rows) ? rows : []) {
    if (!row || typeof row !== "object") continue;
    if (Array.isArray(row.insert)) {
      for (const entry of row.insert) if (entry && typeof entry === "object" && entry.id) ids.push(String(entry.id));
    } else if (row.id) {
...
async function mcpAdd(body, cx) {
  ...
  await withPatchWrite(layout, (list) => {
    ...
    const existingIds = patchEntryIds(list);
...
function registerTavilySearch(ctx, configOf) {
  ...
  if (typeof ctx.effect === "function") {
    ctx.effect(() => disposeAll, "better-deepseek-harness: tavily_search registrations");
  }
...
const routes = {
  ...
  "/ext/api/plugin/install": {
    ...
    handler: async (body, cx) => {
      ...
      const rowIds = rowEntryIds(record.rows);
      const applied = await waitForLoaderState(ctx, (entries) => {
  ...
  "/ext/api/plugin/set-enabled": {
    ...
    handler: async (body, cx) => {
      ...
      const rowIds = rowEntryIds(updatedRows);
      const applied = await waitForLoaderState(ctx, (entries) => {
...
export { NAME, SETTINGS_NS, apply, inject, materializePackage, packageEntryPoints, packageEntryExists, ensureBuiltPackage };
