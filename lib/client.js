window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			tabNotify: "通知",
			notifyTitle: "Windows 通知",
			notifyIntro: "在 Windows 上通过系统 Toast 通知提醒：模型向你提问时、或一次流程结束时（无论成功还是出错）都会弹出通知，即使窗口在后台或未聚焦也能看到。配置保存在 settings.yaml 的 ext-center 节。",
			notifyEnabled: "启用 Windows 通知",
			notifyOnQuestion: "提问时通知",
			notifyOnQuestionHint: "模型需要你输入（ask_user_question）时弹出通知，附问题摘要",
			notifyOnDone: "流程结束时通知",
			notifyOnDoneHint: "一次流程完成或出错时弹出通知，附结果与耗时",
			notifyNote: "仅 Windows 平台生效（通过系统 Toast 显示，无需额外安装）。",
			optimizeTitle: "优化输入",
		...
		var en = {
			...
			notifyTitle: "Windows notifications",
			notifyIntro: "System toast alerts on Windows: a notification pops when the model asks you a question, and when a flow ends (successfully or with an error) — even while the app is in the background or unfocused. Stored in the ext-center section of settings.yaml.",
			notifyEnabled: "Enable Windows notifications",
			notifyOnQuestion: "Notify when asking",
			notifyOnQuestionHint: "Pop a toast when the model needs your input (ask_user_question), with a question summary",
			notifyOnDone: "Notify when a flow ends",
			notifyOnDoneHint: "Pop a toast when a flow completes or fails, with the outcome and duration",
			notifyNote: "Windows only (native system toasts, nothing to install).",
		...
		/** Default client request budget; long-running installs get a larger one. */
		var API_DEFAULT_TIMEOUT_MS = 120000;
		var API_POLL_TIMEOUT_MS = 15000;
		var API_GIT_TIMEOUT_MS = 60000;
		var API_INSTALL_TIMEOUT_MS = 15 * 60 * 1000;
		/** Create an abort signal that always fires, even without AbortSignal.timeout. */
		function createRequestSignal(timeoutMs) {
			...
			if (typeof AbortController !== "undefined") {
				...
				controller.signal.addEventListener("abort", function () { clearTimeout(timer); }, { once: true });
				return controller.signal;
			}
			return void 0;
		}
		async function callApi(path, body, timeoutMs) {
			var budget = typeof timeoutMs === "number" && timeoutMs > 0 ? timeoutMs : API_DEFAULT_TIMEOUT_MS;
			var signal = createRequestSignal(budget);
			var response;
			try {
				...
				if (signal && signal.aborted) {
					var timedOut = new Error("request timed out after " + budget + " ms: " + path);
			...
			var json = await response.json().catch(function () {
				return { ok: false, error: { code: "bad-response", message: "bad response from " + path + " (HTTP " + response.status + ")" } };
			});
			if (!json.ok) {
		...
		function PluginsTab(props) {
			...
			var install = useCallback(function () {
				...
				callApi("/ext/api/plugin/install", { source: source }, API_INSTALL_TIMEOUT_MS).then(function (value) {
					setSpec("");
					setUrl("");
					setFolder("");
					setGit("");
		...
		//#endregion
		//#region lib/types/client/NotifyTab.js
		/** The Notifications tab: Windows toast toggles through /ext/api. */
		function NotifyTab(props) {
			...
			useEffect(function () {
				if (!ready || draft !== null) return;
				var value = state.config || {};
				var notify = value.notify && typeof value.notify === "object" ? value.notify : {};
				setDraft({
					enabled: notify.enabled === true,
					onQuestion: notify.onQuestion === true,
					onDone: notify.onDone === true
				});
			}, [ready, draft, state]);
			var save = useCallback(function () {
				if (!ready || draft === null) return;
				...
				setError(null);
				setMessage(null);
				callApi("/ext/api/config", {
					notify: {
						enabled: !!draft.enabled,
						onQuestion: !!draft.onQuestion,
						onDone: !!draft.onDone
					}
				}).then(function (next) {
					setState(next);
			...
			var reset = useCallback(function () {
				...
				setError(null);
				setMessage(null);
				callApi("/ext/api/config", { reset: ["notify"] }).then(function (next) {
					setState(next);
					setDraft(null);
					setMessage(t("saved"));
				}, function (reason) {
			...
			return jsxs("div", { className: "extc-panel", children: [
				jsxs("div", { className: "extc-card", children: [
					jsx("h3", { className: "extc-card-title", children: t("notifyTitle") }),
					jsx("p", { className: "extc-empty", children: t("notifyIntro") }),
					!writable ? jsx("p", { className: "extc-error", children: t("readOnly") }) : null,
					!ready ? jsx("p", { className: "extc-empty", children: t("loading") }) : draft === null ? jsx("p", { className: "extc-empty", children: t("loading") }) : jsxs("div", { className: "extc-form", children: [
						jsx(Field, { label: t("notifyEnabled"), children: jsx("label", { className: "extc-check", children: [
							jsx("input", { type: "checkbox", checked: draft.enabled, disabled: !writable || busy, onChange: function (event) { setDraft(Object.assign({}, draft, { enabled: event.target.checked })); } }),
							jsx("span", { children: draft.enabled ? t("pluginEnabled") : t("pluginDisabled") })
						] }) }),
						jsx(Field, { label: t("notifyOnQuestion"), hint: t("notifyOnQuestionHint"), children: jsx("label", { className: "extc-check", children: [
							jsx("input", { type: "checkbox", checked: draft.onQuestion, disabled: !writable || busy, onChange: function (event) { setDraft(Object.assign({}, draft, { onQuestion: event.target.checked })); } }),
							jsx("span", { children: draft.onQuestion ? t("pluginEnabled") : t("pluginDisabled") })
						] }) }),
						jsx(Field, { label: t("notifyOnDone"), hint: t("notifyOnDoneHint"), children: jsx("label", { className: "extc-check", children: [
							jsx("input", { type: "checkbox", checked: draft.onDone, disabled: !writable || busy, onChange: function (event) { setDraft(Object.assign({}, draft, { onDone: event.target.checked })); } }),
							jsx("span", { children: draft.onDone ? t("pluginEnabled") : t("pluginDisabled") })
						] }) }),
						jsx("p", { className: "extc-empty", children: t("notifyNote") }),
						jsxs("div", { className: "extc-actions", children: [
							jsx(Button, { primary: true, busy: busy, disabled: !writable, onClick: save, children: t("save") }),
							jsx(Button, { small: true, disabled: !writable || busy, onClick: reset, children: t("reset") + " · " + t("tabNotify") })
						] })
					] }),
					jsx(StatusLine, { error: error, message: message })
				] })
		...
		function TerminalTab(props) {
			...
			var loadList = useCallback(function () {
				callApi("/ext/api/terminal/list", void 0, API_POLL_TIMEOUT_MS).then(function (value) {
			...
			useEffect(function () {
				...
				var timer = window.setInterval(function () {
					callApi("/ext/api/terminal/output?id=" + encodeURIComponent(activeKey) + "&after=" + cursor.current, void 0, API_POLL_TIMEOUT_MS).then(function (value) {
		...
		function GitTab(props) {
			...
			var gitCall = function (path, body) {
				...
				if (body === void 0) return callApi(path, void 0, API_GIT_TIMEOUT_MS);
				var payload = Object.assign({}, body || {});
				if (currentSession) payload.sessionId = currentSession;
				return callApi(path, payload, API_GIT_TIMEOUT_MS);
			};
			...
			var discard = useCallback(function (change, stagedFlag) {
				if (busy !== null) return;
				if (!window.confirm(t("gitDiscardConfirm") + " " + change.path)) return;
				setBusy("discard");
				setError(null);
				setOk(null);
				gitCall("/ext/api/git/discard", { paths: [change.path], staged: stagedFlag === true }).then(function () {
					setBusy(null);
				...
				return jsxs("div", { className: "extc-git-group", children: [
					...
					list.map(function (change) {
						return jsx(GitRow, {
							...
							onDiscard: function () { discard(change, group === "staged"); }
						}, group + ":" + change.path);
					})
				] });
			};
		...
		function McpTab(props) {
			...
			var load = useCallback(function () {
				callApi("/ext/api/mcp/list", void 0, API_POLL_TIMEOUT_MS).then(function (value) {
		...
		function RescueDialog(props) {
			...
			var load = useCallback(function () {
				callApi("/ext/api/rescue/status", void 0, API_POLL_TIMEOUT_MS).then(function (value) {
					setStatus(value);
		...
		function ExtensionCenterSection(props) {
			...
			return jsxs("div", { className: "extc", children: [
				...
				tab === "notify" ? jsx(NotifyTab, { t: t, loadState: props.loadState }) : null,
		...
		function apply(ctx) {
					...
					inject: function (sessionId) {
						return {
							getCurrentModel: function () {
								var models = null;
								try { models = ctx.get("modelDirectories"); } catch { models = null; }
			...
			ctx.effect(function () {
				...
				var surfaceDisposers = { tree: null, git: null, terminal: null };
				var safeRegister = function (register) {
					try { return register(); } catch (error) {
						// A broken registration must never take the GUI down.
						try { ctx.logger?.warn?.("ext-center: surface registration failed", error); } catch { }
						return function () { };
					}
				};
				var disposeSurface = function (surface) {
					if (surfaceDisposers[surface] === null) return;
					try { surfaceDisposers[surface](); } catch { /* a slot disposer must not break unload */ }
					surfaceDisposers[surface] = null;
				};
				var syncSurfaces = function (views) {
					if (disposed) return;
					var suppression = clientDshWebUiSuppression(views);
					var wanted = {
						tree: !suppression.tree,
						git: !suppression.git,
						terminal: !suppression.terminal
					};
					for (var surface of ["tree", "git", "terminal"]) {
						if (wanted[surface]) {
							if (surfaceDisposers[surface] !== null) continue;
							surfaceDisposers[surface] = safeRegister(
								surface === "tree" ? registerTreeAction : surface === "git" ? registerGitTab : registerTerminalTab
							);
						} else {
				...
				var disposeAllSurfaces = function () {
					for (var surface of ["tree", "git", "terminal"]) disposeSurface(surface);
				};
				// Initial decision after the normal 8 s convergence window.
				var gate = waitForClientLoaderSettle(ctx, 8000).then(function (state) {
					if (disposed) return;
					syncSurfaces(state.views);
					// If siblings were still pending at the deadline, keep
					// re-checking the loader until it settles so a late family
					// activation still takes over the surface instead of leaving
					// two registered copies. A permanently pending tree keeps the
					// fail-open surfaces (the timer stops with the plugin effect).
					if (!state.settled) {
						watcherTimer = setInterval(function () {
							var next = clientLoaderState(ctx);
							if (next === null) {
								// Loader disappeared — keep the fail-open decision.
								if (watcherTimer !== null) clearInterval(watcherTimer);
								watcherTimer = null;
								return;
							}
							syncSurfaces(next.views);
							if (next.settled && watcherTimer !== null) {
				...
				gate.catch(function (error) {
					...
					if (disposed) return;
					try { ctx.logger?.warn?.("ext-center: dsh-web-ui gate failed", error); } catch { }
					syncSurfaces([]);
				});
				return function () {
					disposed = true;
