...
/** A plain JSON object (any shape accepted by the tool schema). */
export type JsonObject = Record<string, unknown>;

/**
 * The compiled parameter schema of a tool
 * (`ToolDefinition.parameters`, e.g. `{type:"object", properties, required}`).
 * Only `properties` is read; everything else is schema metadata.
 */
...
/** The repair policy from the plugin config (`cfg.toolRepair`). */
export interface RepairPolicy {
  /** Master switch; `false` passes arguments through untouched. */
  enabled: boolean;
  /** Neutral placeholder for a missing / empty / wrongly-typed `description`. */
  descriptionFill: string;
}

/** The arguments to dispatch and whether they changed. */
 ...
 */
export function tryParseJsonObject(text: string): JsonObject | undefined {
...
export function repairToolArguments(
  ...
  args: unknown,
  policy: RepairPolicy | null | undefined
): RepairResult {
  if (!policy || policy.enabled === false) return { arguments: args, changed: false };
