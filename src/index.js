...
function requireGitRoot(cx) {
  const root = discoverGitRoot(cx);
...
async function gitStatusOf(cx) {
  const root = requireGitRoot(cx);
...
function gitStatus(body, cx) {
  return gitStatusOf(cx).then(({ root, state }) => ({ root, ...state }));
}

async function gitDiff(body, cx, query) {
  const { root, state } = await gitStatusOf(cx);
...
async function gitBranches(body, cx) {
  const root = requireGitRoot(cx);
...
const routes = {
  ...
  "/ext/api/git/status": {
    ...
    mutating: false,
    requiresLocal: true,
    handler: (body, cx) => gitStatus(body, cx)
  },
  "/ext/api/git/diff": {
    method: "GET",
    readsBody: false,
  ...
  "/ext/api/git/branches": {
    ...
    mutating: false,
    requiresLocal: true,
    handler: (body, cx) => gitBranches(body, cx)
  },
  "/ext/api/git/stage": {
    method: "POST",
    readsBody: true,
  ...
  "/ext/api/git/stage-all": {
    ...
    readsBody: true,
    mutating: true,
    handler: (body, cx) => runGit(requireGitRoot(cx), ["add", "-A"], cx.cfg.git.timeoutMs).then(() => ({ ok: true }))
  },
  "/ext/api/git/unstage": {
    method: "POST",
    readsBody: true,
  ...
  "/ext/api/git/unstage-all": {
    ...
    readsBody: true,
    mutating: true,
    handler: (body, cx) => runGit(requireGitRoot(cx), ["reset"], cx.cfg.git.timeoutMs).then(() => ({ ok: true }))
  },
  "/ext/api/git/commit": {
    method: "POST",
    readsBody: true,
