...
/** The state for one terminal's stripped output. */
export interface TerminalBufferState {
  /** Retained bytes (the tail of the stream). */
  buffer: Buffer;
  /** Absolute byte count appended since the session started. */
  bytes: number;
  /** Absolute byte offset of the first complete UTF-8 character in `buffer`. */
  start: number;
}

/** Create an empty terminal output ring. */
export function createTerminalBuffer(): TerminalBufferState {
 ...
 * character.
 */
export function appendTerminalBuffer(state: TerminalBufferState, chunk: string, limit: number): void {
  if (typeof chunk !== "string" || chunk.length === 0) return;
  const data = Buffer.from(chunk, "utf8");
  state.buffer = Buffer.concat([state.buffer, data]);
  state.bytes += data.length;
  let start = Math.max(0, state.buffer.length - limit);
  while (start < state.buffer.length && (state.buffer[start] & 0xc0) === 0x80) start += 1;
 ...
 */
export function terminalBufferSlice(state: TerminalBufferState, after: number): { text: string; cursor: number } {
