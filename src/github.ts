...
export const GITHUB_DEFAULTS: Readonly<GithubSettings> = Object.freeze({
 ...
 */
export function resolveGithubSettings(value: unknown): GithubSettings {
...
function splitRepoSlash(input: string): [string, string] {
  const slash = input.indexOf("/");
  return [input.slice(0, slash), input.slice(slash + 1).replace(/\.git$/, "")];
}

/** Validate one owner or repo name part. */
function validateRepoPart(part: string, label: string): string | null {
  if (part.length > REPO_PART_MAX_LENGTH) return `${label} "${part}" is too long (max ${REPO_PART_MAX_LENGTH} chars)`;
  if (/[\0\r\n]/.test(part)) return `${label} contains control characters`;
  if (!REPO_PART_RE.test(part)) return `${label} "${part}" is invalid — use only letters, digits, ".", "-" and "_"`;
  return null;
}
 ...
 */
export function parseRepoRef(owner: unknown, repo: unknown): { ok: true; ref: RepoRef } | { ok: false; message: string } {
  ...
  let r = repoPart;
  if (r.includes("/")) {
    if (o !== "") return { ok: false, message: 'use either separate owner and repo arguments or a combined "owner/repo", not both' };
    [o, r] = splitRepoSlash(r);
  } else if (o.includes("/")) {
    if (r !== "") return { ok: false, message: 'use either separate owner and repo arguments or a combined "owner/repo", not both' };
  ...
  if (ownerProblem !== null) return { ok: false, message: ownerProblem };
  const repoProblem = validateRepoPart(r, "repo");
  if (repoProblem !== null) return { ok: false, message: repoProblem };
...
export function validateGithubPath(path: string): string | null {
  ...
  if (path.length > GITHUB_ARG_MAX_LENGTH) return `path is too long (max ${GITHUB_ARG_MAX_LENGTH} chars)`;
  if (/[\0\r\n]/.test(path)) return "path contains control characters";
...
export function githubContentsUrl(owner: string, repo: string, path: string, ref: string | undefined): string {
...
export function githubSearchUrl(query: string, perPage: number): string {
  return `${GITHUB_API_URL}/search/repositories?q=${encodeURIComponent(query)}&per_page=${perPage}`;
}

/** Releases endpoint. */
export function githubReleasesUrl(owner: string, repo: string, perPage: number): string {
 ...
 */
export function mapGithubRepoResponse(json: unknown): GithubRepoInfo {
  ...
  const license = typeof body.license === "object" && body.license !== null
    && typeof (body.license as Record<string, unknown>).spdx_id === "string"
    ? (body.license as Record<string, unknown>).spdx_id as string
    : void 0;
  return {
    fullName,
...
export type GithubContentsResult =
  | { kind: "dir"; entries: GithubTreeEntry[]; truncated: boolean }
  | { kind: "file"; content: string; size: number; truncated: boolean; binary: boolean };
 ...
 */
export function mapGithubContentsResponse(json: unknown, cap: number = GITHUB_FILE_CONTENT_CAP): GithubContentsResult {
  if (Array.isArray(json)) {
    const entries: GithubTreeEntry[] = [];
    for (const entry of json) {
      if (entries.length >= GITHUB_TREE_ENTRIES_CAP) break;
      if (typeof entry !== "object" || entry === null) continue;
      const row = entry as Record<string, unknown>;
      const name = typeof row.name === "string" ? row.name : "";
      const type = row.type === "dir" ? "dir" : row.type === "file" ? "file" : void 0;
      if (name === "" || type === void 0) continue;
      const item: GithubTreeEntry = {
        name,
        path: typeof row.path === "string" && row.path !== "" ? row.path : name,
        type
      };
      if (typeof row.size === "number") item.size = row.size;
      if (typeof row.download_url === "string" && row.download_url.length > 0) item.downloadUrl = row.download_url;
      entries.push(item);
    }
 ...
 */
export function mapGithubSearchResponse(json: unknown, cap: number = GITHUB_SEARCH_CAP): GithubSearchResult {
  const body = typeof json === "object" && json !== null ? json as Record<string, unknown> : {};
  ...
  for (const entry of rawItems) {
    ...
    if (typeof row.description === "string" && row.description.length > 0) item.description = row.description;
    if (typeof row.language === "string" && row.language.length > 0) item.language = row.language;
    items.push(item);
  }
 ...
 */
export function mapGithubReleasesResponse(
  json: unknown,
  cap: number = GITHUB_RELEASES_CAP,
  bodyCap: number = GITHUB_RELEASE_BODY_CAP
): GithubReleasesResult {
  const raw = Array.isArray(json) ? json : [];
  const releases: GithubRelease[] = [];
  for (const entry of raw) {
    if (releases.length >= cap) break;
    if (typeof entry !== "object" || entry === null) continue;
    const row = entry as Record<string, unknown>;
    ...
    if (typeof row.name === "string" && row.name.length > 0) release.name = row.name;
    if (typeof row.published_at === "string" && row.published_at.length > 0) release.publishedAt = row.published_at;
 ...
 */
export function githubErrorMessage(status: number, body: unknown, resource: string): string {
...
export function formatGithubRepoOutput(info: GithubRepoInfo): string {
  const lines = [
  ...
  if (info.description !== void 0 && info.description.length > 0) lines.push(`description: ${info.description}`);
  const extra = [
...
export function formatGithubTreeOutput(result: { entries: GithubTreeEntry[]; truncated: boolean }): string {
  if (result.entries.length === 0) return "The directory is empty.";
  const lines = result.entries.map((entry) => {
    const size = entry.type === "file" && entry.size !== void 0 ? ` (${entry.size} bytes)` : "";
    return `${entry.type === "dir" ? "[dir] " : ""}${entry.path}${size}`;
  });
  if (result.truncated) lines.push(`…[listing truncated at ${GITHUB_TREE_ENTRIES_CAP} entries]`);
  return lines.join("\n");
}

/** Format file content as the model-facing text block (with truncation/binary notices). */
export function formatGithubFileOutput(result: { content: string; size: number; truncated: boolean; binary: boolean }): string {
  if (result.binary) {
...
export function formatGithubSearchOutput(result: GithubSearchResult): string {
  if (result.items.length === 0) return "No repositories found.";
...
export function formatGithubReleasesOutput(result: GithubReleasesResult): string {
  if (result.releases.length === 0) return "No releases found.";
