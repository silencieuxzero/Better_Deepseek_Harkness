...
/** One chunk's stripped text plus the trailing incomplete escape prefix. */
export interface AnsiStripResult {
  /** Visible text (escape sequences removed). */
  text: string;
  /** Trailing bytes that may be the start of an escape sequence. */
  tail: string;
}
...
function isPartialEscape(rest: string): boolean {
  if (rest === "\x1b") return true;
  if (rest.startsWith("\x1b[")) return !CSI.test(rest);
  if (rest.startsWith("\x1b]")) return true; // OSC runs until BEL or ST
  if (rest.startsWith("\x1b(") || rest.startsWith("\x1b)")) return rest.length < 3;
  return false;
}
 ...
 */
export function stripAnsiChunk(chunk: string, tail = ""): AnsiStripResult {
  ...
  while (at < input.length) {
    ...
    if (isPartialEscape(rest)) return { text, tail: rest };
    text += input[at];
    at += 1;
  }
  return { text, tail: "" };
