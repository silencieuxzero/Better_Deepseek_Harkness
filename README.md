...
### 方式一：手动安装（无需 pnpm / npm）

把 `better-deepseek-harness` 整个目录复制到共享模块根：

```powershell
Copy-Item -Recurse better-deepseek-harness "$HOME\.dsh\profiles\node_modules\better-deepseek-harness"
```
...
- 插件安装 = 包落到 `~/.dsh/profiles/node_modules`（profile 解析链上的共享根）→ 把该包 `dsh.bundle.patch` 的行（若有）合并进 `cordis.patch.yml`；没有 bundle 补丁的包自动补一条 `{id: <包名>, name: <包名>}` 行，保证它能被加载 → HMR 配置监听器事务性重放补丁，条目即时挂载
