...
- **cordis.patch.yml 是加载器树的唯一活源**：所有变更（插件安装/停用/卸载、MCP 行、急救模式的手动触发与 settle 修订）都经同一个事务性写入器：解析 → 合并 → 临时文件 + rename 原子写，保留文件头注释，`!!js` 表达式（loader 配置方言）往返无损。连续写入串行化，避免配置监听器背靠背刷新。唯一的例外是启动看门狗：它必须同步抢在启动审计前完成首次急救写入。
...
- 请求体 2 MiB、技能 URL 下载 1 MiB、插件 tgz 下载 64 MiB、树文件读取/写入 1 MiB、终端单次写入 4096 字符、git 单批路径 500 条、归档删除单批 500 条、输入优化单次文本 100 KiB；
